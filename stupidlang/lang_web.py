...
def array_make(name,*items):
  ...
  arrays.append(temp)
def read_f(name):
  output_method('[ERROR] file reading not supported in browser')
def run_f(name):
  global fns
  for fn in fns:
    if fn.split(':')[0] == name:
...
def run_script(code):
  global is_doing_shit, variables, fns, arg_funcs, arrays, pairings, _output
  is_doing_shit = True
  variables = []
  fns = []
  arg_funcs = []
